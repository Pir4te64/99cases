...
import { productsPersonalizadas } from "./FundasPersonalizadasGET";
...
const ProductsPersonalizadasVerTodos: React.FC<ProductsProps> = ({
  visibleTitle = true,
}) => {
  const [visibleCount, setVisibleCount] = useState(4);

  const handleLoadMore = () => {
    setVisibleCount(productsPersonalizadas.length);
  };

  const displayedProducts = productsPersonalizadas.slice(0, visibleCount);
  ...
  return (
    <div className="bg-white mx-auto py-8 px-4">
      ...
      {visibleCount < productsPersonalizadas.length && (
...
export default ProductsPersonalizadasVerTodos;
